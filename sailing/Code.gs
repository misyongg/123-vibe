...
var SYSTEM_PROMPT_ = [
  ...
  '키워드에 있는 사실만, 빠짐없이, 정해진 칸에 넣습니다. 짧게 뭉개거나 없는 내용을 만들지 않습니다.',
  '',
  '【공통】',
  '1. 공공기록물 기준: 객관·구체. 내담자 발언은 직접 표현 유지.',
  '2. journal(일지)=사실만. memo(메모)에서만 상담자 견해.',
  '3. 키워드에 없는 차회 목표·활동·유의사항·준비물을 만들지 말 것. 차회일시만 있으면 일시만 기록.',
  '4. 빈 “-”, “(해당 내용 없음)”, “생각을 이끌어냈다”, “과정을 확인함” 금지.',
  '5. journal·memo는 JSON string. 객체/배열 금지. 줄바꿈은 \\n.',
  '6. 본문에 「상담일지: n회기」「상담자 메모: n회기」「이름」 줄을 쓰지 말 것.',
  '',
  '【일지 칸 분류 — 엄수. 틀리면 실패】',
  '내담자 관찰 = 외견·졸림·눈맞춤·말투·태도·기억 여부 등 객관 관찰만.',
  '내담자 행동 = 주어가 내담자인 말·점수·만들기·놀이·자작 이야기 장면·내담자의 답만.',
  '상담활동 = 주어가 상담자(또는 함께 실시)인 활동: 그림책 읽기, 활동 진행, 상담자 질문.',
  '',
  '【잘못된 예 → 고친 예】',
  '× 행동: 토리의 용기여행 세번째 이야기를 읽고 모래놀이를 시작하였다.',
  '○ 상담활동: 그림책 「토리의 용기여행」 세 번째 이야기를 읽음. / 모래놀이 상담을 진행함.',
  '× 행동: 이야기가 이어지며 친구가 되어가는 것을 요약하고 질문하였다.',
  '○ 상담활동: 상담자 질문 「어떻게 해서 시작이 된걸까? 어떤 용기가 필요했을까?」',
  '○ 행동: 내담자 답 「토토가 안녕이라고 했어요.」',
  '× 행동: 책을 읽고 클레이·모래놀이를 하였다. (상담자 주도 활동을 행동에 넣음)',
  '○ 행동에는 클레이로 ○○를 만듦 / 이야기 장면만. 읽기·진행·질문은 상담활동.',
  '',
  '차회상담계획 = 키워드에 차회일시/계획이 있을 때만. 있는 내용만.',
  '',
  '【일지 양식】',
  '일시- YYYY.MM.DD.(요일) n교시(소요시간 ○○분)  또는  일시- YYYY.MM.DD.(요일) HH:MM~HH:MM(소요시간 ○○분)',
  '장소- (일정 구분 값)',
  '',
  '내담자 관찰',
  '- …',
  '',
  '내담자 행동',
  '- 발언·점수·클레이·만들기 태도 (주어=내담자)',
  '- 내담자가 만든 이야기는 다음과 같음:',
  '  - (장면마다 하위 불릿. 음식·식탁·분식집 등 키워드 장면 누락 금지)',
  '- 내담자 답: 「…」',
  '',
  '상담활동',
  '- 그림책 읽기: 제목·회차. 줄거리가 키워드에 있으면 사실로 한 줄.',
  '- 클레이 활동 실시',
  '- 모래놀이 상담 진행',
  '- 상담자 질문: 「…」 문구 그대로.',
  '',
  ...
  '- 사례번호: (제공값)',
  '',
  '일정- YYYY.MM.DD.(요일) n교시(○○분)  또는  HH:MM~HH:MM(○○분)',
  '',
  '상담활동 요약',
  '- 책·점수·발언·클레이·모래놀이 장면 핵심·질문·답을 구체 불릿으로. 이야기 전개를 한 줄로 뭉개지 말 것.',
  '',
  '상담자 견해',
  '- 정서 상태: …',
  '- 관계 특성: …',
  ...
  '- 보호요인: …',
  '- 현재 상담 초점: …',
  '※ 5줄 필수. 키워드 근거만. 놀이 속 싸움을 실제 또래 갈등으로 단정하지 말 것.',
  '※ 위험 근거가 없으면 「키워드상 위기·자해 발언은 보고되지 않음」처럼 사실형으로.',
  '',
  '차회 계획',
  '※ 키워드에 목표·활동·유의·준비물이 있을 때만. 차회일시만 있으면 아래만:',
  '차회일시(n+1회기)',
  '- …',
  '',
  'JSON: { "journal": "...", "memo": "..." }'
...
function buildUserPrompt_(ctx) {
  ...
  var lines = [
    '[메타데이터]',
    '회기: ' + (ctx.sessionLabel || ''),
    '사례번호: ' + (ctx.caseNo || '') + ' (memo 「- 사례번호:」만)',
    '이름: ' + (ctx.name || '') + ' (memo에 이름 넣지 말 것)',
    '성별: ' + (ctx.gender || ''),
    '학교명: ' + (ctx.school || ''),
    '학년/반: ' + (ctx.gradeClass || ''),
    '대상: ' + (ctx.target || '학생'),
    ...
    '교시: ' + (ctx.period || ''),
    '일정 구분: ' + (ctx.category || ''),
    '장소(일지용·일정 구분): ' + journalPlace,
    '유형: ' + (ctx.type || ''),
    '',
    '[키워드 — 상담 메모·차회일시·과제 모두 여기. 사실 근거, 빠짐없이]',
    ctx.keywords || '(없음)',
    '',
    '작성 지시:',
    '- journal 첫 줄 「일시-」. 장소- = 일정 구분 값.',
    '- 내담자 행동 = 주어가 내담자인 내용만. 읽기·진행·요약·질문은 상담활동.',
    '- 상담활동에 그림책 읽기·모래놀이 진행·상담자 질문 문구를 넣을 것.',
    '- 점수·발언·이야기 장면(음식·식탁 등)·차회일시 누락 금지.',
    '- memo 견해 5줄 필수. 차회 목표·준비물은 키워드에 있을 때만.',
    '- journal·memo string JSON.'
  ];
  if (ctx.previousSummary) {
    lines.push('', '[이전 회기 참고 — 1줄만. 복사 금지]', ctx.previousSummary);
  }
  return lines.join('\n');
}
