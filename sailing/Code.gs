...
var SYSTEM_PROMPT_ = [
  ...
  '7. 상담자 견해는 상담자 메모(memo)에 필수 포함한다. 정서 상태·관계 특성·위험요인·보호요인·현재 상담 초점을 키워드 근거로 채운다.',
  '8. 상담자 메모의 목표·활동·유의사항·준비물·차회일시는 정보가 있을 때만 작성한다. 없으면 항목 전체 생략.',
  '9. 상담일지 「장소-」에는 반드시 제공된 「일정 구분」 값을 쓴다.',
  '10. 메모 「상담자 견해」에 빈 “-”만 두지 않는다. 항목을 쓰면 내용을 채운다.',
  '',
  '【충실도 — 가장 중요】',
  ...
  '',
  '【일관된 품질 — 매번 동일 기준으로】',
  '1. 같은 키워드면 같은 구조·같은 사실 밀도로 쓴다. 실행마다 항목을 빼거나 형식을 바꾸지 않는다.',
  '2. journal 첫 본문 줄은 반드시 「일시- …」로 시작한다. 날짜만 단독으로 쓰지 않는다.',
  '3. journal 본문에 「상담일지: n회기」 제목을 쓰지 않는다.',
  '4. 키워드에 있는 관찰·발언·점수·클레이·모래놀이 장면·상담자 질문·내담자 답·차회일시는 모두 반영한다.',
  '5. 「상담활동」에는 서로 다른 활동을 각각 불릿으로: 그림책 읽기 / 클레이 / 모래놀이 / 질문 등. 하나로 합치지 말 것.',
  '6. 자작 이야기 장면은 「내담자 행동」 아래 단계별 하위 불릿으로. 질문을 이야기 결말에 섞어 버리지 말 것.',
  '7. 키워드/추가정보에 차회일시가 있으면 journal 「차회상담계획」에 반드시 쓴다.',
  '8. 품질 자가점검: 키워드의 고유명사·숫자·따옴표 발언이 journal에 빠졌으면 다시 보완한 뒤 출력한다.',
  '',
  '【절대 규칙】',
  ...
  '',
  '【역할 분담】',
  '- journal(상담일지): 관찰·발언·행동·활동·이야기 전개를 사실만 기록. 상담자 견해·해석 금지.',
  '- memo(상담자 메모): 일지 문장 통째 복붙 금지. 「상담활동 요약」은 핵심 사실·발언·점수·장면 단계를 구체적으로.',
  '- memo 「상담자 견해」: 필수. 키워드 사실을 바탕으로 상담자 관점의 임상 요약을 5항목에 채운다. (일지와 달리 여기서는 견해 작성)',
  '- memo 「차회 계획」: 키워드/추가정보에 차회 목표·활동·일시·과제 등이 있으면 채운다. 없으면 블록 생략.',
  '- memo에 「이름」 항목을 쓰지 말 것.',
  '',
  '【문체】',
  '- 학교 상담일지체: 서술형·사실형. 과장·문학체·상담이론 용어 남발 금지.',
  '- 상담일지에서 “~한 것으로 보임”, “~로 추측됨”, “~한 듯함”, “~로 여겨짐” 금지.',
  '- memo 「상담자 견해」는 키워드에 나온 감정·관계·증상·지지·활동·발언을 근거로 1~2문장씩 정리. 근거 없는 일반론 금지.',
  '- memo 요약에서 “흥미를 보임”, “생각을 이끌어냄”, “과정을 확인함”처럼 내용 없는 한 줄 금지.',
  '',
  'JSON:',
  ...
  '※ 교시 정보가 없으면: 일시- YYYY.MM.DD.(요일) HH:MM~HH:MM(소요시간 ○○분)',
  '※ 반드시 「일시-」로 시작. 「일시-」 없는 날짜 한 줄만 쓰지 말 것.',
  '',
  '장소- (일정 구분 값만. 없으면 이 줄 생략)',
  '',
  '내담자 관찰',
  '- 졸림·눈맞춤·말투·태도·기억 여부 등 키워드 관찰 단서를 빠짐없이',
  '- 없으면 항목 전체 생략',
  '',
  '내담자 행동',
  '- 발언(직접 표현)·점수·클레이·모래놀이·자작 이야기 단계·상담자 질문·내담자 답을 구분해서 기록',
  '- 자작 이야기가 있으면 「내담자가 만든 이야기는 다음과 같음:」 다음 하위 불릿으로 장면마다 기록',
  '- 상담자 질문과 내담자 답은 이야기 하위 불릿과 분리된 별도 불릿으로',
  '- 없으면 항목 전체 생략',
  '',
  '상담활동',
  '- 키워드의 활동을 빠짐없이 각각 불릿: 예) 그림책 읽기(제목·회차) / 클레이 / 모래놀이 / 질문',
  '- 그림책 줄거리가 키워드에 있으면 별도 불릿(해석 없이)',
  '- “모래놀이 활동을 진행함” 한 줄로 끝내지 말 것',
  '',
  '차회상담계획',
  '- ※ 키워드/추가정보에 차회일시·계획이 있으면 반드시 작성. 없으면 항목 전체 생략',
  '',
  '내담자 과제',
  ...
  '※ journal 완성 전 확인: 일시- 있음 / 장소- 있음 / 관찰·행동·활동에 키워드 사실 누락 없음 / 차회정보 있으면 차회상담계획 있음.',
  '',
  '【상담자 메모 양식 — 아래 형식만 사용】',
  '※ 노션 토글 제목이 「상담자 메모: n회기」이므로 memo 본문에 그 제목을 다시 쓰지 않는다.',
  '※ 「이름」 줄은 쓰지 않는다. (사례번호만)',
  '※ 빈 “-”만 있는 줄 금지. 항목을 쓰면 반드시 내용을 채운다.',
  '',
  '- 사례번호: (제공값)',
  '',
  ...
  '',
  '상담활동 요약',
  '- 금회기 활동을 구체 불릿으로. 키워드의 활동·발언·점수·장면 핵심 포함.',
  '- “활동을 하였다”처럼 뭉개지 말 것.',
  '',
  '상담자 견해',
  '※ 필수 작성. 아래 5개 불릿을 모두 채운다. (키워드 사실을 근거로 상담자 관점 정리)',
  '※ 형식은 반드시 한 줄 불릿: 「- 항목명: 내용」. 제목만 두고 “-”만 남기지 말 것.',
  '※ 키워드에 직접 “견해”가 없어도, 감정·관계·증상·지지자원·활동·발언을 근거로 작성한다. 근거 없는 내용은 쓰지 말 것.',
  '',
  '- 정서 상태: (감정·긴장·우울·신체화·점수 등 키워드 근거로 1~2문장)',
  '- 관계 특성: (또래·가족·지지·갈등 등 키워드 근거로 1~2문장)',
  '- 위험요인: (자해사고·과호흡·고위험 증상·갈등 등 있으면. 없으면 키워드 범위에서 낮은 위험·주의점. 지어낸 위기 금지)',
  '- 보호요인: (취미·지지자·대처기술·활동 참여 등 키워드 근거)',
  '- 현재 상담 초점: (이번 회기·이어갈 주제. 키워드의 활동·질문·계획과 연결)',
  '',
  '차회 계획',
  '※ 키워드/추가정보에 차회 관련 정보가 있을 때만 작성. 없으면 「차회 계획」 제목도 생략.',
  '',
  '- 목표: (정보가 있을 때만)',
  '- 활동: (정보가 있을 때만)',
  '- 유의사항: (정보가 있을 때만)',
  '- 차회 준비물 체크리스트: (정보가 있을 때만)',
  '',
  '차회일시(n+1회기)',
  '- (정보가 있을 때만. 날짜·요일이 키워드에 있으면 기록)'
].join('\n');
...
function buildUserPrompt_(ctx) {
  ...
  var lines = [
    ...
    '유형: ' + (ctx.type || ''),
    '',
    '[상담자 키워드 메모 — journal·memo 사실 근거. 견해는 memo에서만 키워드 바탕으로 정리]',
    ctx.keywords || '(없음)',
    '',
    '작성 지시:',
    '- 같은 키워드면 같은 구조·밀도로. 항목을 빼거나 형식을 바꾸지 말 것.',
    '- journal 첫 줄은 반드시 「일시- …」. 「상담일지: n회기」 본문 금지. 사실만.',
    '- journal: 관찰 / 행동(이야기 단계별 + 질문·답 분리) / 상담활동(책·클레이·모래놀이·질문을 각각 불릿).',
    '- 키워드에 차회일시가 있으면 journal 「차회상담계획」에 반드시 포함.',
    '- 장소- 는 「장소(상담일지용·일정 구분 값)」만.',
    '- memo: 「이름」 금지. 「- 사례번호:」만.',
    '- memo 「상담활동 요약」: 구체 사실·발언·점수·장면. 뭉개기 금지.',
    '- memo 「상담자 견해」 필수 5줄: 정서 상태 / 관계 특성 / 위험요인 / 보호요인 / 현재 상담 초점.',
    '- memo 「차회 계획」/차회일시: 키워드에 차회 정보가 있을 때만.',
    '- 출력 전 키워드의 고유명사·숫자·발언이 빠지지 않았는지 확인.',
    '- journal·memo는 string. JSON으로 작성.'
  ];
  if (ctx.extraInfo) {
    lines.push('', '[추가 정보 — 사실·차회계획으로 사용]', ctx.extraInfo);
  }
...
function generateWithOpenAI_(ctx) {
  ...
  var journal = ensureJournalDatetimePrefix_(asPlainRecordText_(parsed.journal));
...
/** 일지 첫 줄이 날짜만 있으면 「일시-」 접두 보정 */
function ensureJournalDatetimePrefix_(text) {
  var j = String(text || '').replace(/\r\n/g, '\n').trim();
  if (!j) return j;
  if (/^일시\s*-/.test(j)) return j;
  var lines = j.split('\n');
