...
var SYSTEM_PROMPT_ = [
  ...
  '5. 키워드를 자연스러운 문장으로 바꾸되, 사실·활동·말을 보태지 않는다. 키워드에 나온 고유명사·활동명은 유지한다.',
  '6. 차회 일정·목표·활동·유의사항·준비물·과제는 키워드/추가정보에 있을 때만 쓴다.',
  '7. 상담자 의견은 키워드에 의견·소감이 있을 때만 쓴다. 없으면 항목 생략.',
  ...
  '',
  '상담일지 양식(순서 유지. 값 없는 선택 항목은 통째로 생략):',
  '회기 :',
  '사례번호 :',
  '이름 :',
  '성별 :',
  ...
  '',
  '1. 내담자 관찰',
  '(키워드에 관찰 단서가 있을 때만. 없으면 항목 전체 생략)',
  '',
  '2. 내담자 행동',
  '(키워드에 행동 단서가 있을 때만. 없으면 생략)',
  '',
  '3. 상담활동',
  '(키워드의 활동·프로그램을 중심으로 작성)',
  '4. 차회상담계획',
  '',
  '5. 내담자 과제',
  '',
  '6. 기타사항',
  '',
  '※ 1~3은 키워드를 분류해 쓰되, 키워드가 활동 위주면 3에 집중하고 1·2는 키워드에 근거가 있을 때만 짧게.',
  '※ 4~6은 정보가 있을 때만.',
  '',
  ...
  '',
  '일시',
  'YYYY.MM.DD.(요일) HH:MM~HH:MM (○○분)',
  '',
  '상담내용',
  '(이번 회기 핵심만 3~6문장. 일지 복붙 금지)',
  '',
  '상담자 의견',
  '(키워드에 의견이 있을 때만)',
  '',
...
function buildUserPrompt_(ctx) {
  var lines = [
    '[메타데이터 — 값이 있는 것만 기록에 기입. 빈 값은 칸을 비우거나 생략]',
    '회기: ' + (ctx.sessionLabel || ''),
    '사례번호: ' + (ctx.caseNo || ''),
    '이름: ' + (ctx.name || ''),
    '성별: ' + (ctx.gender || ''),
    ...
    '- 상담일지 내용 1~3은 키워드를 쪼개 짧게. 같은 문장을 메모에 반복하지 말 것.',
